...
(def reveal-animation?
  "When false, the picker paints the full task list on the first frame and runs
   none of the reveal-animation mechanics (no tick command, no per-task phases)."
  false)
...
(defn- picker-init
  ...
  (let [state {:tasks         (vec tasks)
               ...
               :term-height   24
               :animation-phase (if reveal-animation? :cta :done)
               :animation-index 0}]
    (if reveal-animation?
      [state (tasks-animation-tick-cmd tasks-animation-frame-rate)]
      [state nil])))
